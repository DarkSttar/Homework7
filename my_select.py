...
def query_1(session=session, limit=5):
    query = (
        session.query(
            Student.id.label("StudentID"), Student.name, func.avg(Grade.grade)
        )
...
def query_4(session=session):
    query = session.query(func.avg(Grade.grade)).all()
    return query


def query_5(session=session, professorID=1):
    query = (
        session.query(Professor.name, Subject.name)
...
def query_6(session=session, groupID=1):
    query = (
        session.query(Student.id, Student.name, Group.name)
        .join(Student, Student.group_id == Group.id)
...
def query_7(session=session, groupID=1, subjectID=1):
    query = (
        session.query(Subject.name, Group.name, Student.id, Student.name, Grade.grade)
        .join(Student, Student.group_id == Group.id)
        .join(Grade, Grade.student_id == Student.id)
        .join(Subject, and_(Subject.id == Grade.subject_id, Subject.id == subjectID))
        .where(Group.id == groupID)
...
def query_8(session=session, professorID=1):
    query = (
        session.query(Professor.id, Professor.name, Subject.name, func.avg(Grade.grade))
        .join(Subject, Subject.professor_id == Professor.id)
...
def query_9(session=session, studentID=1):
    query = (
        session.query(Student.id, Student.name, Subject.name)
        .join(Grade, Grade.student_id == Student.id)
...
def query_10(session=session, professorID=1, studentID=1):
    query = (
        session.query(Student.id, Student.name, Professor.name, Subject.name)
        .join(Grade, Grade.student_id == Student.id)
        .join(Subject, Subject.id == Grade.subject_id)
        .join(
            Professor,
            and_(Professor.id == Subject.professor_id, Professor.id == professorID),
        )
        .where(Student.id == studentID)
